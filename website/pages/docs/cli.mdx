...
Depending you usage, you can install `@svgr2/cli` locally in your project:

```sh
npm install --save-dev @svgr2/cli
# or use yarn
yarn add --dev @svgr2/cli
```

or use it on the fly using `npx @svgr2/cli`.
...
```sh
npx @svgr2/cli -- icons/clock-icon.svg
```

### From standard input

`@svgr2/cli` supports standard input, just redirect file contents using `<` operator.
...
```sh
npx @svgr2/cli -- icons/clock-icon.svg > dist/ClockIcon.js
```
...
```sh
# Usage: npx @svgr2/cli [--out-dir dir] [--ignore-existing] [src-dir]
$ npx @svgr2/cli --out-dir dist -- icons
...
```sh
npx @svgr2/cli --out-dir dist --ignore-existing -- icons
```
...
```sh
npx @svgr2/cli --out-dir dist --filename-case kebab -- icons
```
...
```sh
npx @svgr2/cli --out-dir dist --no-index -- icons
```
...
```sh
npx @svgr2/cli  --out-dir dist --index-template index-template.js -- icons
```
