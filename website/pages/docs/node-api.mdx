...
```bash
npm install --save-dev @svgr2/core
# or use yarn
yarn add --dev @svgr2/core
```

## Usage

Import `transform` from `@svgr2/core` to transform a file. It takes three arguments:
...
By default `@svgr2/core` doesn't include `svgo` and `prettier` plugins. If you need these, you have to install it and to specify it in options.

```bash
npm install @svgr2/plugin-jsx @svgr2/plugin-prettier
```
