...
pub struct Options {
    ...
    pub jsx_runtime_import: Option<OptionsJsxRuntimeImport>,
    pub import_source: Option<String>,
}

#[napi(object)]
pub struct OptionsJsxRuntimeImport {
    pub source: String,
...
fn into_expand_props(value: JsExpandProps) -> Result<ExpandProps, TransformError> {
    match value {
...
#[napi]
pub fn transform(source: String, options: Option<Options>) -> napi::Result<String> {
    let options = match options {
        Some(options) => options.into_transform_options(),
        None => Ok(TransformOptions::default()),
    }
